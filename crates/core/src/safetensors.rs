...
impl Shard {
    ...
    pub fn tensors(&self) -> SafeTensors<'_> {
        SafeTensors::deserialize(&self.mmap).expect("invalid safetensors header")
    }
}

// ─── All shards of a model directory ──────────────────────────────────────
...
impl Weights {
    pub fn open(dir: impl AsRef<Path>) -> Result<Self, BoxError> {
        ...
        paths.sort();
        if paths.is_empty() {
            return Err(format!("no .safetensors in {}", dir.as_ref().display()).into());
        }
    ...
    pub fn f32(&self, name: &str) -> Result<Vec<f32>, BoxError> {
        self.with(name, |v| to_f32(&v))
    }

    pub fn quant(&self, name: &str) -> Result<QTensor, BoxError> {
        self.with(name, |v| QTensor::from_view(&v))
    }
    ...
    fn with<T>(&self, name: &str, f: impl FnOnce(TensorView<'_>) -> T) -> Result<T, BoxError> {
        ...
        let tensors = self.shards[id].tensors();
        Ok(f(tensors.tensor(name)?))
    }
}
